// refcodec.js — gputex CPU reference block codecs (BC1 / BC4 / BC5 / BC7 modes 1,4,6 / ASTC 4x4 LDR / ETC2 RGB8).
...
// NOTE: decodeBC7Block covers modes 1/4/6 (throws on 0/2/3/5/7) and decodeASTC4x4Block covers
// gputex's own subset only (single partition, block modes 0x253/0x242/0x053/0x042, CEM 0/8/12;
// throws on spark's ASTC config) — bench.js scores BC7 with bc7full.js (modes 4/5/6) and ASTC
// with the hardware decoder, using these as cross-checks on gputex's output.
...
function fitClass(cls, pixels8) {
  ...
  let e0 = quantEndpoint(cls, Array.from({ length: C }, (_, c) => vals[fp.i0 * C + c]));
  let e1 = quantEndpoint(cls, Array.from({ length: C }, (_, c) => vals[fp.i1 * C + c]));
  ...
  if (cls.endpointRange === 192) {
    ...
    for (let g = 0;g < values.length; g += 5) {
      const grp = values.slice(g, g + 5).map((v) => {
        ...
        if (q < 0)
          throw new Error(`packBlock: ${v} is not a QUANT_192 level`);
        return q;
      });
      const t = [0, 0, 0, 0, 0];
...
var SUPPORTED_MODES = new Set([BLOCK_MODE_4x4_2BIT, BLOCK_MODE_4x4_3BIT, BLOCK_MODE_4x4_4BIT, BLOCK_MODE_4x4_5BIT]);
...
function fastSearch(D) {
  let mx = 0;
  for (let i = 0;i < D.length; i++)
    mx = Math.max(mx, Math.abs(D[i]));
  let cover = 0;
  while (cover < 7 && B3[cover] < mx)
    cover++;
...
function encodeFastBlock(px) {
  ...
  const constErr = (s, b) => s.sq - 2 * (b[0] * s.sum[0] + b[1] * s.sum[1] + b[2] * s.sum[2]) + 8 * (b[0] * b[0] + b[1] * b[1] + b[2] * b[2]);
  const Dof = (texels, lb) => texels.map((k) => luma[k] - lb);
  const evalCodes = (flip, s0, s1, diff, c) => {
  ...
  for (let c = 0;c < 3; c++) {
    ...
    planarEst += sq - 2 * (eO * sA + eH * sB + eV * sC) + 3.5 * (eO * eO + eH * eH + eV * eV) + 0.5 * eO * eH + 0.5 * eO * eV + 4.5 * eH * eV;
  }
